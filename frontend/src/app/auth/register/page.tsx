...
export default function RegisterPage() {
  ...
  useEffect(() => {
    api.get<{ enabled: boolean; client_id: string }>('/api/settings/google-login-enabled')
      .then(data => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await register({ name, email, password, nationality: nationality || undefined });
      router.push('/');
    } catch (err) {
    ...
    <div className="min-h-screen flex pb-16 md:pb-0">
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-white dark:bg-dark-main relative">
        ...
        <div className="w-full max-w-md">
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">{t('auth.register')}</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative">
              ...
              <Input type="password" placeholder={t('auth.password')} value={password} onChange={(e) => setPassword(e.target.value)} className="pl-10" required />
            </div>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" />
              <Input type="password" placeholder={t('auth.confirm_password')} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="pl-10" required />
            </div>
            <div className="relative">
              <Globe className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" />
              <Input placeholder={t('auth.nationality')} value={nationality} onChange={(e) => setNationality(e.target.value)} className="pl-10" />
            </div>
            ...
            <div className="mt-6">
              <div className="relative">
                <div className="absolute inset-0 flex items-center">
                  <div className="w-full border-t border-gray-200 dark:border-gray-500/40" />
                </div>
                <div className="relative flex justify-center text-sm">
                  <span className="px-2 bg-white dark:bg-dark-main text-gray-500">{t('auth.social_login') || 'Or'}</span>
                </div>
              </div>
              ...
              <div className="mt-4">
                ...
                >
                  ...
                  {googleLoading ? t('common.loading') : (t('auth.google') || 'Sign up with Google')}
                </Button>
