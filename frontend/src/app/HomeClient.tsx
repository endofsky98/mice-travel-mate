...
export default function HomeClient({ initialData }: HomeClientProps) {
  ...
  const [banners, setBanners] = useState<BannerType[]>(initialData.banners);
  ...
  const [festivals, setFestivals] = useState<Festival[]>(initialData.festivals);
  ...
  // Refetch on language change or event slug change (skip initial if data already loaded)
  useEffect(() => {
    if (!isLoaded) return;
  ...
  // Location change -> refetch restaurants only
  useEffect(() => {
    if (!isLoaded) return;
